...
with open('cur_compilation.py', 'wt') as f:
	if 'clientVersion' in sys.argv:
		pathPrefix = 'client/'
		IS_CLIENT = True
		IS_SERVER = False
		f.write("IS_CLIENT = True\n")
		f.write("IS_SERVER = False\n")
	elif 'serverVersion' in sys.argv:
		pathPrefix = 'server/'
		IS_CLIENT = False
		IS_SERVER = True
		f.write("IS_CLIENT = False\n")
		f.write("IS_SERVER = True\n")
	else:
		pathPrefix = 'full/'
		IS_CLIENT = True
		IS_SERVER = True
...
# |
	...
	if WRECK.generate_item_modifiers: export['item_modifiers'] = pathPrefix + 'Data/item_modifiers.txt'
	if WRECK.generate_ui_strings: export['ui_strings'] = commonPrefix + 'Languages/en/ui.csv'
	if WRECK.generate_user_hints: export['user_hints'] = commonPrefix + 'Languages/en/hints.csv'
